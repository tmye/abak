...
public class ForgotPasswordActivity extends AppCompatActivity implements ForgotPasswordContract.View, View.OnClickListener {
    ...
    private ForgotPasswordActivity.SmsBroadcastListener br;
    ...
    private void runApp() {
        ...
        forgotPasswordPresenter = new ForgotPasswordPresenter(this, customerDataRepository);
        ...
        /* get informations */
        String account_no = getIntent().getStringExtra(PHONE_NUMBER);
...
//        boolean is_registered = getIntent().getBooleanExtra(IS_REGISTERED, false);
        ...
        if (!"".equals(password)) {
            forgotPasswordPresenter.recoverPassword(account_no,password, request_id);
        }
    }
    ...
    ForgotPasswordActivity.CodeDiscountThread discountThread;
    ...
    private void initSendCodeBasics() {
        ...
        if (diff > MIN_ELLAPSE_TIME) {
            ...
            discountThread = new ForgotPasswordActivity.CodeDiscountThread();
    ...
    private void initEdittextDrawables() {
        ...
        ed_code.setCompoundDrawablesWithIntrinsicBounds(drawable_password, null, null, null);
    }
...
/*
    ...
    private String getRequestId() {
        SharedPreferences pref = getSharedPreferences(Config.PRE_RECOVER_PASSWORD_SHARED_PREFS, MODE_PRIVATE);
        /* check if the code expired or not */
    ...
    public void codeIsOk(boolean isOk) {
        ...
        if (isOk) {
            ...
            intent.putExtra(ConfirmPayActivity.TRANS_TYPE, ConfirmPayActivity.RESET_PASSWORD);
            intent.putExtra(ConfirmPayActivity.STEP_COUNT, 1);
            /* give it request id and phonenumber, and nickname*/

            intent.putExtra(ForgotPasswordActivity.PHONE_NUMBER, ed_phone.getText().toString());

            if (!"".equals(request_id)) {
                intent.putExtra(ForgotPasswordActivity.REQUEST_ID, request_id);
                startActivity(intent);
                finish();
            } else {
    ...
    private void launchResetPassword() {
        /* start activities and reset the password if the code is true */
    }
    ...
    private void sendVerificationCode() {
        ...
        if (canISendCode()) {
            ...
            forgotPasswordPresenter.sendVerificationCode(current_phone_number);
        } else {
...
//        forgotPasswordPresenter.check_verification_code(code, request_id);
    }
    ...
    protected void onStart() {
        ...
        br = new ForgotPasswordActivity.SmsBroadcastListener();
